...
<template>
    <div class="coming_soon">
        <div v-if="!coming_soonList">
            <el-table v-loading="loading" element-loading-text="拼命加载中" style="width: 100%">
            </el-table>
        </div>
       <div v-if="coming_soonList">
            <div class="clearfix">
                <h2 class="fl">{{ coming_soonList.title }}</h2>
            </div>
            <el-card class="box-card">
                <div v-for="(item,index) in coming_soonList.subjects" class="text item">
      ...
      <div class="pagination" v-if="coming_soonList">
         <el-pagination
...
export default {
    ...
    computed: {
        coming_soonList() {
            return this.$store.state.coming_soon.coming_soonList
        }
    },
   
    mounted () {
        if(!this.coming_soonList) {
            this.getComing_soon( {
                count: 10,
                start: 0
            })
    ...
    methods: {
        getComing_soon(params) {
            this.$store.dispatch('getComing_soon',params)
        },
